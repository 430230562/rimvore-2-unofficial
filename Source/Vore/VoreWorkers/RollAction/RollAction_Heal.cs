...
namespace RimVore2
{
    public class RollAction_Heal : RollAction
    {
        ...
        private bool HealPawn(float rollStrength)
        {
            ...
            injury.Severity -= quality * TargetPawn.InjuryHealingFactor;
            if(RV2Log.ShouldLog(false, "OngoingVore"))
                RV2Log.Message("Healed " + TargetPawn.Label + "'s hediff " + injury.def.label + " by " + quality + "*" + TargetPawn.InjuryHealingFactor, "OngoingVore");
