...
namespace RimVore2
{
    ...
    public class StruggleManager : IExposable
    {
        ...
        private bool NoStruggleDueToHealth()
        {
            PawnCapacitiesHandler capacities = record.Prey.health.capacities;
            if(capacities == null)
            {
                return false;
            }
            if(capacities.CapableOf(PawnCapacityDefOf.Moving))
            {
                if(RV2Log.ShouldLog(false, "Struggling"))
                    RV2Log.Message($"Prey {record.Prey.LabelShort} can't struggle, not capable of moving", false, "Struggling");
                return true;
            }
